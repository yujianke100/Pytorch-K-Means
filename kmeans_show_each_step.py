...
def k_means(data, k, color_box, max_time=MAX_LOOP):
    plt.close(1)
    global loop_time
    ...
    for loop_time in range(max_time):
        ...
        if((labels == last_labels).all()):
            color = labels.astype(np.float64).tolist()
            color = [(lambda x: color_box[int(x)])(i) for i in color]
            plt.figure('result')
            plt.scatter(data[:, 0], data[:, 1], c=color)
            plt.title('result k={} finished with {} loop'.format(
                k, loop_time))
            plt.show()
            break
        last_labels = labels
        ...
        plt.figure('result')
        plt.scatter(data[:, 0], data[:, 1], c=color)
        plt.title('result k={} with {} loop'.format(
            k, loop_time))
        plt.show()
        plt.pause(0.01)
